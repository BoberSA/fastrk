        ...
        def test_events(self):
            ...
            for tbl in self.tables:
                ...
                arr, ev = rk_prop_ev(crtbp, self.s0, self.t0, self.t1, np.inf, self.rtol, self.atol,
                                     ...
                                     call_event, self.atol, self.rtol, 100, self.mc)
                # event count
                self.assertTrue(ev.shape[0] == 1133, f"ev_count:{ev.shape[0]}")
                # eventX
                self.assertTrue(np.allclose(ev[ev[:, 0] == 0, 4], 0., rtol=self.rtol, atol=self.atol), f"ev=\n{ev}")
                # eventY
                self.assertTrue(np.allclose(ev[ev[:, 0] == 1, 4], 0., rtol=self.rtol, atol=self.atol), f"ev=\n{ev}")
